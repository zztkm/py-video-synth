"""オリジナルソースコード: https://github.com/burningion/pygame-video-synthesizer/blob/master/video-synthesizer.py 。"""
...
# onsetdd = aubio.onset("default", win_s, hop_s, samplerate)
...
def draw_pygame():
    ...
    while running:
        ...
        if not q.empty():
            b = q.get()
            newCircle = Circle(random.randint(0, screenWidth), random.randint(0, screenHeight), random.choice(colors), 700)
...
def get_onsets():
    while True:
        try:
            ...
            if onset.onset_detect(y=signal, sr=samplerate, hop_length=hop_s):
                q.put(True)
